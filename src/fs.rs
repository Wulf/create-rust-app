use crate::logger::{modify_file_msg, file_msg};
  ...
  if !directory.exists() {
    let proceed = match prompt_before_create {
      ...
      true => Confirm::with_theme(&ColorfulTheme::default())
        .with_prompt(format!("Directory does not exist, create '{:#?}' directory?", &directory))
        .default(false)
        .interact()?
    };
    ...
    if proceed {
      ...
      return Err(anyhow!("Required directory '{:#?}' was not present!", &directory));
    }
  } 
  
  if !directory.is_dir() {
    let proceed = Confirm::with_theme(&ColorfulTheme::default())
      .with_prompt(format!("Expected directory but found file at '{:#?}'.", &directory))
      .default(false)
      .interact()?;
    ...
    if proceed {
      ...
      return Err(anyhow!("Found file '{:#?}' but expecte directory!", &directory));
    }
  }
