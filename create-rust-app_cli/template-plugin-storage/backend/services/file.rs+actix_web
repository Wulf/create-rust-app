...
async fn all(db: Data<Database>, storage: Data<Storage>) -> HttpResponse {
    ...
    let files = Attachment::find_all_for_record(&db, "file".to_string(), "NULL".to_string(), 0).unwrap_or_default();
    let blob_ids = files.iter().map(|f| f.blob_id).collect::<Vec<_>>();
    let blobs = AttachmentBlob::find_all_by_id(&db, blob_ids).unwrap_or_default();

    let mut files = blobs.iter().enumerate().map(|b| FileInfo {
        id: files[b.0].id,
        key: b.1.clone().key,
        name: b.1.clone().file_name.unwrap_or("".to_string()),
        url: None,
...
async fn delete(db: Data<Database>, storage: Data<Storage>, file_id: Path<i32>) -> HttpResponse {
    ...
    let detach_op = Attachment::detach(&db, &storage, file_id).await;
    ...
    while let Some(item) = payload.next().await {
        ...
        match field_name {
            "file" => {
                ...
                let attached_req = Attachment::attach(&db, &store, "file".to_string(), "NULL".to_string(), 0, AttachmentData {
                    data,
                    file_name
                }, true, false).await;
