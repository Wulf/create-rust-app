...
// #[get("/mail")]
// async fn get_mailers() -> HttpResponse {
//     let mail_dir = concat!(env!("CARGO_MANIFEST_DIR"), "/backend/mail/");
//     let mail_dir = PathBuf::from(mail_dir);
//
//     if mail_dir.exists() && mail_dir.is_dir() {
//         // mail dir exists
//         return HttpResponse::Ok().json(r#"{"success": true, "mail": []}"#)
//     }
//
//     HttpResponse::Ok().json(serde_json::from_str(r#"{"success": false, "mail": []}"#))
// }

#[post("/db/query")]
async fn query_db(db: Data<Database>, body: Json<MySqlQuery>) -> HttpResponse {
    let use_raw_query = body.raw.unwrap_or(false);
    let q = if use_raw_query { body.query.clone() } else { format!("SELECT json_agg(q) as json FROM ({}) q;", body.query) };
    let db = db.pool.get().unwrap();

    if use_raw_query {
        let exec = sql_query(q.as_str()).execute(&db);
        return if exec.is_err() {
            HttpResponse::InternalServerError().finish()
        } else {
            HttpResponse::Ok().json(exec.unwrap())
        }
    }
