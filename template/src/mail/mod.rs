...
impl Mailer {
  ...
  pub fn check_environment_variables() {
    ...
    if std::env::var("SMTP_FROM_ADDRESS").is_err() {
        println!("Note: SMTP_FROM_ADDRESS environment variable is not set, no mail will be sent!");
    }
    ...
    if std::env::var("SEND_EMAIL").is_err() || std::env::var("SEND_MAIL").unwrap().eq("true") {
        println!("Note: SEND_MAIL environment variale is not set to \"true\", no mail will be sent!");
    }
  }
