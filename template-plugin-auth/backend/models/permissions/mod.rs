...
use diesel::{RunQueryDsl, sql_query, sql_types::{Integer, Text}};

use crate::{DB, models::permissions::user_permission::{UserPermission, UserPermissionChangeset}};
use serde::{Serialize, Deserialize};

use self::{role_permission::{RolePermission, RolePermissionChangeset}, user_role::{UserRole, UserRoleChangeset}};
...
impl Permission {
  ...
  pub fn grant_to_user(db: &DB, user_id: ID, permission: &str) -> Result<bool> {
    let granted = UserPermission::create(&db, &UserPermissionChangeset {
  ...
  pub fn grant_to_role(db: &DB, role: &str, permission: &str) -> Result<bool> {
    let granted = RolePermission::create(&db, &RolePermissionChangeset {
      permission: permission.to_string(),
      role: role.to_string()
    });
  ...
  pub fn assign_role(db: &DB, user_id: ID, role: &str) -> Result<bool> {
    let assigned = UserRole::create(&db, &UserRoleChangeset {
        user_id: user_id,
        role: role.to_string(),
    });
