...
impl UserRole {
  ...
  pub fn read(db: &DB, item_user_id: ID, item_role: String) -> QueryResult<Self> {
      ...
      user_roles.filter(user_id.eq(item_user_id).and(role.eq(item_role))).first::<UserRole>(db)
  }
  ...
  pub fn delete(db: &DB, item_user_id: ID, item_role: String) -> QueryResult<usize> {
      ...
      diesel::delete(user_roles.filter(user_id.eq(item_user_id).and(role.eq(item_role)))).execute(db)
  }
}
