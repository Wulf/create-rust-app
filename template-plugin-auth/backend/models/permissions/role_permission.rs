...
impl RolePermission {
  ...
  pub fn read(db: &DB, item_role: String, item_permission: String) -> QueryResult<Self> {
      ...
      role_permissions.filter(role.eq(item_role).and(permission.eq(item_permission))).first::<RolePermission>(db)
  }
  ...
  pub fn delete(db: &DB, item_role: String, item_permission: String) -> QueryResult<usize> {
      ...
      diesel::delete(role_permissions.filter(role.eq(item_role).and(permission.eq(item_permission)))).execute(db)
  }
}
